...
if (isset($_GET['userid'])) {
    $investmentId = $_GET['userid'];

    // Fetch the user's investment details, including walletDuration
    $query = "SELECT * FROM userinvest WHERE userid = '$investmentId'";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $walletDuration = $investment['walletDuration'];

        if ($walletDuration !== null) {
            // Set the start date as today (confirmation date)
            $startDate = new DateTime();
            $endDate = clone $startDate;
            $endDate->modify("+$walletDuration days");

            // Calculate profit (5x the investment amount)
            $profit = $amount * 5;

            // Update the investment status to 'running', set the start date and end date, and calculate profit
            $updateStatus = "UPDATE userinvest SET status = 'running', start_date = '" . $startDate->format('Y-m-d') . "', end_date = '" . $endDate->format('Y-m-d') . "', profit = '$profit' WHERE userid = '$investmentId'";
            $conn->query($updateStatus);

            // Send a notification to the user about the investment and end date
            $message = "Your investment of $$amount has been confirmed and is now running. It will mature on " . $endDate->format('Y-m-d') . " with an expected profit of $$profit.";
            $insertMessage = "INSERT INTO notifications (userid, message) VALUES ('$userId', '$message')";
            $conn->query($insertMessage);

            // Redirect with success message
            $_SESSION['message'] = "Investment confirmed and set to running. User notified. The investment will mature on " . $endDate->format('Y-m-d') . " with an expected profit of $$profit.";
            $_SESSION['message_type'] = "success";
            header("Location: investdetails?userid=$userId");
            exit();
        } else {
            $_SESSION['message'] = "Investment duration is missing.";
            $_SESSION['message_type'] = "error";
            header("Location: investment");
            exit();
        }
    } else {
        $_SESSION['message'] = "Investment not found.";
        $_SESSION['message_type'] = "error";
        header("Location: investment");
        exit();
    }
